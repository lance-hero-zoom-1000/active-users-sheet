...
def update_regular(drange, period):
    ...
    for i, custom_date in enumerate(drange):
        ...
        metric_calculator.update_sheet(active_users_df,MetricCalculator.active_users_dict, format=format)
        metric_calculator.update_sheet(rdp_views_df, bounds_dict=MetricCalculator.rdp_viewed_users_dict, format=format)
        metric_calculator.update_sheet(dopay_users_df, bounds_dict=MetricCalculator.dopay_users_dict, format=format)
        metric_calculator.update_sheet(dopay_trans_df, bounds_dict=MetricCalculator.dopay_transactions_dict, format=format)
        metric_calculator.update_sheet(dp_members_df, bounds_dict=MetricCalculator.dp_users_in_db_dict, format=format)
        metric_calculator.update_sheet(dp_active_df, bounds_dict=MetricCalculator.dp_active_users_dict, format=format)
        metric_calculator.update_sheet(dp_redeemed_df, bounds_dict=MetricCalculator.dp_redemption_users_dict, format=format)
        metric_calculator.update_sheet(no_redemptions_df, bounds_dict=MetricCalculator.dp_redemptions_dict, format=format)
